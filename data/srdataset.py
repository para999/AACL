...
class SRDataset(Dataset):
    ...
    def __init__(self, args, name, train=True, sim=False):
    ...
    def get_sim_patch(self, hr, var_threshold_min=0, var_threshold_max=800):
        scale = self.args.scale[0]
        out = []
        # hr = common.augment(hr)

        while True:
            hr_patch_1 = common.get_patch(hr, patch_size=self.args.patch_size, scale=scale)
            hr_patch_2 = common.get_patch(hr, patch_size=self.args.patch_size, scale=scale)

            var_1 = np.var(hr_patch_1)
            var_2 = np.var(hr_patch_2)

            if var_threshold_min < abs(var_1 - var_2) < var_threshold_max:
                # print(abs(var_1 - var_2))
                out.append(hr_patch_1)
                out.append(hr_patch_2)
                break
        return out