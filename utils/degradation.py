...
#         self.noise = torch.rand(1).cuda() * (self.noise_max - self.noise_min) + self.noise_min
#         noise_level = torch.rand(int(B / 2), 1, 1, 1, 1).cuda() * self.noise
...
class IsoBlur(object):
    ...
    def __call__(self, hr_tensor):
        with torch.no_grad():
            ...
            hr_blured = self.blur(hr_tensor, b_kernels)  # .view(-1, C, H, W)
            # print("hr blured shape", hr_blured.shape)
            hr_blured = hr_blured.reshape(B, -1, C, H, W)
            # print("lr noised shape", lr_noised.shape)
            lr = torch.clamp(hr_blured.round(), 0, 255)
...
class ComplexDegradation(object):
    def __init__(self, args):
        self.scale = args.scale
        self.kernel_size = args.kernel_size
        ...
        self.jpeg_min = args.jpeg_min
        self.jpeg_max = args.jpeg_max
    ...
    def __call__(self, hr_tensor):
        with torch.no_grad():
            ...
            # add noise
            # lr_downsampled = lr_downsampled.reshape(B, -1, H // int(self.scale), W // int(self.scale))
            lr_noised = self.noise(lr_downsampled)  # BN C W H
            # print("lr noised shape", lr_noised.shape)
            # lr_noised = lr_noised.reshape(-1, C, W // int(self.scale), H // int(self.scale))
            # print("lr noised shape", lr_noised.shape)
            # jpeg
...
class JPEGDegradation(object):
    def __init__(self, theta, sigma_min, sigma_max, noise, jpeg):
        self.scale = 4
        kernel_size = 21
        blur_type_list = ['iso_gaussian', 'aniso_gaussian']
        blur_type_prob = [0, 1]
        self.gen_kernel = BatchStableKernel(blur_type_list, blur_type_prob, kernel_size, 0.0,
                                            theta, sigma_min, sigma_max)
        self.blur = BatchBlur(kernel_size)
        ...
        self.jpeg = jpeg
        self.jpeger = StableJpeg(mode=255)
    ...
    def __call__(self, hr_tensor):
        with torch.no_grad():
            ...
            lr_noised = lr_noised.reshape(-1, C, W // int(self.scale), H // int(self.scale))
            # jpeg
            lr_jpeged = self.jpeger(lr_noised, self.jpeg)
            # print("lr jpeged shape", lr_jpeged.shape)
            lr_jpeged = torch.clamp(lr_jpeged.round(), 0, 255)
            lr = lr_jpeged.reshape(B, N, C, H // int(self.scale), W // int(self.scale))
            # print("lr shape", lr.shape)
