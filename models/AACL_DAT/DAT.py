...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def calculate_mask(self, H, W):
        ...
        # https://github.com/microsoft/Swin-Transformer/blob/main/models/swin_transformer.py calculate attention mask
        # for shift window
...
class DATB(nn.Module):
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
        ...
        self.cdim1 = RCDIM(dim)
        self.cdim2 = RCDIM(dim)
    ...
    def forward(self, x, dp, x_size):
        ...
        """
        H, W = x_size
        x = x + self.drop_path(self.attn(self.cdim1(self.norm1(x), dp), H, W))
        x = x + self.drop_path(self.ffn(self.cdim2(self.norm2(x), dp), H, W))
