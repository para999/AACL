...
import numpy as np
import einops
from models.AACL_IPG.ipg_kit import flex, cossim, local_sampling, global_sampling
...
class DF_conv(nn.Module):
    def __init__(self, channels_in, channels_out, kernel_size, reduction):
        super(DF_conv, self).__init__()
    ...
    def forward(self, x):
        ...
        # branch 1
        kernel = self.kernel(x[1]).reshape(-1, 1, self.kernel_size, self.kernel_size)
        out = self.relu(F.conv2d(x[0].reshape(1, -1, h, w), kernel, groups=b * c, padding=(self.kernel_size - 1) // 2))
        out = self.conv(out.reshape(b, -1, h, w))
...
class GAL(nn.Module):
    ...
    def forward(self, x, x_size, graph):
        ...
        # x = self.df_conv(x.permute(0, 3, 1, 2), dp).permute(0, 2, 3, 1).contiguous()
    ...
    def flops(self):
        ...
        if self.conv_scale != 0:
            flops += W * self.conv_block.flops(self.window_size * self.window_size)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, x_size, graph, dp):
        short_cut = x
        x = self.cdim(self.norm(x), dp) + short_cut
